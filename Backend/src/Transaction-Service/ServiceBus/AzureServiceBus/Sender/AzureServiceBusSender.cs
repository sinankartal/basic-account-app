...
public class AzureServiceBusSender : IServiceBusSender
{
    ...
    public async Task SendMessageAsync(string message, object data, string correlationId)
    {
        ...
        {
            // _logger.LogError($"Send message failed for {message} since it is too large.");
        ...
        try
        {
            ...
            // _logger.LogError($"Message: {message} has been published to the queue.");
        }
        finally
        {
            await sender.DisposeAsync();
            await client.DisposeAsync();
            // _logger.LogError($"Client and sender were disposed.");
        }
    }
}